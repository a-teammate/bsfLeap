...
namespace bs
{
	...
	struct LeapFinger
	{
		...
		union {
			...
			struct {
				...
				* For thumbs, this bone is set to have zero length and width, an identity basis matrix, and its joint
				* positions are equal.
				*/
	...
	struct LeapPalm
	{
		...
		* content. The stabilized position lags behind the palm position by a variable amount, depending primarily
		...
		*/
		...
		/**
		* The normal vector to the palm. If your hand is flat, this vector will point downward, or "out" of the front
		* surface of your palm.
		*/
		Vector3 mNormal;
		...
		* {normal x direction, -normal, -direction}
		*/
	...
	struct LeapHand
	{
		...
		*/
		...
		/**
		 * The normalized estimate of the pinch pose. Zero is not pinching; one is fully pinched.
		 */
		float mPinchStrength;

		/**
		* The normalized estimate of the grab hand pose. Zero is not grabbing; one is fully grabbing.
		*/
		float mGrabStrength;
		...
		/** The arm to which this hand is attached. An arm consists of a single LeapBone struct. */
		LeapBone mArm;
	};
	...
	*/
	struct LeapFrame
	{
		...
		/**
		* An identifier for this tracking frame. This identifier is meant to be monotonically increasing, but values may be
		* skipped if the client application does not poll for messages fast enough. This number also generally increases at
		* the same rate as info.frame_id, but if the server cannot process every image received from the device cameras, the
		...
		*
		* This frame rate is distinct from the image frame rate, which is the rate that images are being read from the
		* device. Depending on host CPU limitations, the tracking frame rate may be substantially less than the device frame
		* rate.
		*
		* This number is generally equal to or less than the device frame rate, but there is one case where this number may
		* be _higher_ than the device frame rate:  When the device rate drops. In this case, the device frame rate will fall
		* sooner than the tracking frame rate.
		*
